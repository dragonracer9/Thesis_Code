 ...
 * @return ret_t
 */
[[nodiscard]] inline ret_t Stepper::init_steppers()
{
 ...
 * @return ret_t
 */
[[nodiscard]] ret_t Stepper::move_steps(const uint8_t motor_index, const uint32_t steps,
    const dir_t dir)
{
...
{
    return (
        uint32_t)((angle / ((double)2 * PI)) * (double)steps_per_revolution); // i have no idea if this is correct
 ...
 * @return ret_t
 */
[[nodiscard]] ret_t Stepper::move_angle(const uint8_t motor_index, const double angle,
    const dir_t dir)
{
