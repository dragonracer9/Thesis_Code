...
//    Serial.println(" ");
    ...
    if(motorIndex >= 0 && motorIndex < NUM_MOTORS) {
        switch(markerType) {
            case 0: handleContinuousRotationMarker(motorIndex); break;
            case 1: setRotationSpeed(motorIndex); break;
            case 2: setStepsNumber(motorIndex); break;
            case 3: handleSteppedRotation(motorIndex); break;
        }
    } else {
...
void handleContinuousRotation(int motorIndex) {
    if (!rotationEnabled[motorIndex]) return;
